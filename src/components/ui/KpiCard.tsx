...
export default function KpiCard({ title, value, subText, icon: Icon, colorClass = "text-blue-600 bg-blue-50 border-blue-100", onClick }: KpiCardProps) {
  return (
    <div 
      onClick={onClick}
      className={`bg-[#1a1a1a] rounded-2xl border border-slate-800 p-5 shadow-xs flex flex-col justify-between glass-panel dashboard-card ${onClick ? "cursor-pointer" : ""}`}
    >
      ...
      <div className="mt-4">
        <h3 className="text-2xl font-bold text-slate-200 font-mono tracking-tight">{value}</h3>
