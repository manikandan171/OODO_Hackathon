  ...
  const getStatusBadge = (status: TripStatus) => {
    switch (status) {
      case TripStatus.DRAFT:
        return (
          <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full bg-[#111] text-slate-600 border border-slate-700 uppercase font-mono">
            Draft
          </span>
        );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-200">Route Assignment Control</h1>
      ...
      <div className="bg-[#1a1a1a] rounded-xl border border-slate-800 p-4 shadow-xs grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-[#1a1a1a] text-slate-200 border-slate-700 focus:border-blue-500 text-sm pl-9 pr-4 py-2 rounded-lg border border-slate-700 focus:outline-hidden focus:border-blue-500 transition"
          />
        </div>
      ...
      <div className="space-y-4">
        {filteredTrips.map((t) => {
          ...
          return (
            <div 
              key={t.id} 
              className={`bg-[#1a1a1a] rounded-2xl border p-5 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 transition hover:border-slate-600 shadow-xs custom-glow ${
                t.status === TripStatus.DISPATCHED ? "border-blue-200 bg-blue-50/5" : "border-slate-800"
              }`}
            >
              {/* Destination / Source Route Path with dynamic visual lines */}
              <div className="space-y-3 shrink-0 w-full lg:w-1/3">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-bold font-mono text-slate-400 uppercase bg-[#1a1a1a] px-2 py-0.5 rounded-sm">
                    Route #{t.id}
                  </span>
                  {getStatusBadge(t.status)}
                </div>
                <div className="flex items-center gap-2.5">
                  <div className="font-bold text-slate-300 text-base">{t.source}</div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full lg:w-1/3">
                {/* Vehicle card block */}
                <div className="p-3 bg-[#111] rounded-xl border border-slate-800/50">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Assigned Vehicle</span>
                  <span className="font-bold text-slate-300 text-sm block mt-1">
              ...
              <div className="flex flex-col sm:flex-row items-stretch lg:items-center justify-end gap-3 w-full lg:w-1/4">
                ...
                {t.status === TripStatus.COMPLETED && (
                  <div className="text-right w-full font-mono text-xs text-slate-500 space-y-1 bg-emerald-50/20 border border-emerald-100/50 p-3 rounded-xl">
                    ...
                    <div>Fuel: <span className="font-bold text-slate-400">{t.fuelConsumedL}L</span> logged</div>
                    <div>Odo final: <span className="font-bold text-slate-400">{t.endOdometerKm} km</span></div>
                  </div>
                )}
                    ...
                    {t.status === TripStatus.DRAFT && (
                      <>
                        <button
                          ...
                          className="w-full bg-blue-600 hover:bg-blue-700 text-slate-200 text-xs font-bold px-3 py-2.5 rounded-lg transition text-center shadow-xs cursor-pointer"
                        >
                          Dispatch Route
                        </button>
                        <button
                          ...
                          className="w-full bg-[#1a1a1a] hover:bg-rose-100 text-slate-600 hover:text-rose-700 text-xs font-bold px-3 py-2.5 rounded-lg border border-slate-700 hover:border-rose-200 transition text-center cursor-pointer"
                        >
                          Cancel Draft
                        </button>
                      </>
                    ...
                    {t.status === TripStatus.DISPATCHED && (
                      <>
                        <button
                          ...
                          className="w-full bg-emerald-600 hover:bg-emerald-700 text-slate-200 text-xs font-bold px-3 py-2.5 rounded-lg transition text-center shadow-xs cursor-pointer"
                        >
                          Complete Route
                        </button>
                        <button
                ...
                {!isDispatcher && (t.status === TripStatus.DRAFT || t.status === TripStatus.DISPATCHED) && (
                  <span className="text-xs text-slate-400 font-medium font-mono border border-slate-700 bg-[#111] px-2 py-1.5 rounded-lg block text-center w-full">
                    ACTIVE ROUTE
                  </span>
                )}
              </div>
        ...
        {filteredTrips.length === 0 && (
          <div className="py-16 bg-[#1a1a1a] border border-slate-800 rounded-2xl text-center text-slate-400 shadow-xs">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-[#1a1a1a] rounded-2xl max-w-lg w-full border border-slate-800 shadow-2xl shadow-black/50 overflow-hidden animate-in fade-in zoom-in duration-150">
            <div className="px-6 py-5 border-b border-slate-800 flex justify-between items-center bg-[#111]">
              <h2 className="text-lg font-bold text-slate-200">Draft Route Assignment</h2>
              <button 
